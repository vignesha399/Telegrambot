...
const fs = require('fs');
let cryptoCurrencyCoinList



async function findCryptoPrice(ctx, userName) {
    try {
        console.log(JSON.parse(require('../DB/cryptoCurrencyCoinList.json')))
        cryptoCurrencyCoinList = require('../DB/cryptoCurrencyCoinList.json');
    } catch (error) {
                ...
                .then(response => {
                    ...
                    if (rate != undefined && response != null && rate) {
                        ...
                        try {
                            ...
                            fs.appendFile('./DB/errorlog.txt', Buffer.from(JSON.stringify(error, null, 2)));
                            updateCryptoCurrencyCoinList();
                            rej(sendMessage(ctx.message.chat.id, 'Please make sure your coin name is correct or not\\! ' + error));
                        }
                    }
                })
        })
